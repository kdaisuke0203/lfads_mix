...
class LFADS_Net(nn.Module):
    ...
    def __init__(self, inputs_dim, T, dt,
        ...
        self.fc_logrates = nn.Sequential(nn.Linear(self.factors_dim,90),nn.ReLU(),
                                          nn.Dropout(0.2),
                                  nn.Linear(90,90),nn.ReLU(),
                                         nn.Dropout(0.2),
                                  nn.Linear(90,self.inputs_dim))
        # -----------
        # Dropout layer
        # -----------
...
#         pdb.set_trace()
    ...
    def generate(self, x):
        ...
        for t in range(self.T):
            ...
            econ_and_fac = torch.cat((self.efcon[:, t+1].clone(), self.ebcon[:,t].clone()), dim = 1)
            ...
            self.kl_loss = 0.0*self.kl_loss + .0*KLCostGaussian(self.u_mean, self.u_logvar,
            ...
            if t>0:
                ...
                #self.klf_loss = 0.0*self.klf_loss + 0.0*KLCostGaussian(f_his_mean[t], f_his_logvar[t],
                                            #self.f_prior_mean1, self.f_prior_logvar1)/x.shape[0]
                self.klf_loss = self.klf_loss + 0.1*KLCostGaussian(f_his_mean[t], f_his_logvar[t],
                                            self.f_prior_mean, self.f_prior_logvar)/x.shape[0]
                for i in range(b_size):
                    if x[i,t,3] >0:
                        self.smo_loss +=  0.0*x[i,t,3]*((f_his[t][i][0]-f_his[t-1][i][0])**2).sum()
                    else:
                        self.smo_loss +=  0.0*((f_his_mean[t]-f_his_mean[t-1])**2).sum()
                    self.dir_loss += 0.0*(abs(f_his[t][i][1]+2)*(x[i,t,8]) +abs(f_his[t][i][0]+2)*(x[i,t,2]+x[i,t,10]) + 
                                        abs(f_his[t][i][0]-2)*(x[i,t,4]+x[i,t,14])+abs(f_his[t][i][1]-2)*(x[i,t,21]+x[i,t,28])).sum()
            #print("x",x[:,t,:],"WW",x[1,t,0])
            #self.dir_loss += 10*(abs(self.f[:,1] - 100)).sum() 
            # Generate rates from factor state
            self.r = torch.clamp(torch.exp(self.fc_logrates(self.f)), min=0.0, max=100)
            #self.r = torch.clamp(self.fc_logrates(self.f), min=0.01, max=50)
    ...
    def fit(self, train_dataset, valid_dataset,
        ...
        save_loc = '%s/models_k09/%s/%s/checkpoints/'%(output, self.dataset_name, self.run_name)
        ...
        if use_tensorboard:
            tb_folder = '%s/models_k09/%s/%s/tensorboard/'%(output, self.dataset_name, self.run_name)
        ...
        for epoch in range(max_epochs):
            ...
            for i, x in enumerate(train_dl, 0):
                ...
                loss = self.recon_loss + kl_weight * self.kl_loss + l2_weight * l2_loss + kl_weight *self.klf_loss + self.smo_loss+ kl_weight *self.dir_loss
                ...
                train_smo_loss += self.smo_loss.data 
            ...
            print('recon: %4d, kl: %5d, dir: %.2f, smo: %4d, klf: %4d, klw: %3f' %(self.recon_loss, kl_weight * self.kl_loss, self.dir_loss, self.smo_loss, kl_weight *self.klf_loss,kl_weight))

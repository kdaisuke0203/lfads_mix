...
class LFADS_Net(nn.Module):
    ...
    def __init__(self, inputs_dim, T, dt,
        ...
        self.fc_logrates = nn.Sequential(nn.Linear(self.factors_dim,600),nn.ReLU(),
                                          nn.Dropout(0.2),
                                  nn.Linear(600,400),nn.ReLU(),
                                  nn.Linear(400,self.inputs_dim))
...
#         pdb.set_trace()
    ...
    def generate(self, x):
        ...
        for t in range(self.T):
            ...
            # Update generator
            self.g = torch.clamp(self.gru_generator(self.g,self.g), min=0.0, max=self.clip_val)
    ...
    def fit(self, train_dataset, valid_dataset,
        ...
        save_loc = '%s/models_no/%s/%s/checkpoints/'%(output, self.dataset_name, self.run_name)
        ...
        if use_tensorboard:
            tb_folder = '%s/models_no/%s/%s/tensorboard/'%(output, self.dataset_name, self.run_name)
        ...
        df.to_csv('%s/models_no/%s/%s/loss.csv'%(output, self.dataset_name, self.run_name), index_label='epoch')

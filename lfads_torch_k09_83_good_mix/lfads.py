        ...
        for t in range(self.T):
            ...
            if t>0:
                for i in range(b_size):
                    self.dir_loss += dir_cof*(abs((f_his[t][i][0]-f_his[t-1][i][0])/(f_his[t-1][i][0]+1e-6))*(x[i,t,31]) +
                                        abs((f_his[t][i][1]-f_his[t-1][i][1])/(f_his[t-1][i][1]+1e-6))*0).sum()
    ...
    def fit(self, train_dataset, valid_dataset,
        ...
        for epoch in range(max_epochs):
            ...
            for i, x in enumerate(train_dl, 0):
                ...
                loss = self.recon_loss + 0*(kl_weight * self.kl_loss + l2_weight * l2_loss +  kl_weight* self.dir_loss)
            ...
            print('recon: %4d, kl: %5d, dir: %4d, klw:%4d' %(self.recon_loss, kl_weight * self.kl_loss, self.dir_loss,self.cost_weights['kl']['weight']))
